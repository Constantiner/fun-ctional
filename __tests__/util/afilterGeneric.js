import afilterGeneric from "../../src/util/afilterGeneric";
...
describe("afilterGeneric tests", () => {
	it("should work for base case", async () => {
		...
		const result = await afilterGeneric()(filterFn)(inputValue);
	...
	it("should work for base case with list", async () => {
		...
		const inputValue = [input1, input2, input3];
		const filterFn = getMockFn(jest)(n => n < 10, "filterFn");
		const result = await afilterGeneric()(filterFn)(new Set(inputValue));
	...
	it("should work for promise resolved to list in parallel", async () => {
		...
		const result = await afilterGeneric()(filterFn)(createAsyncPromise(n => new Set(n))(inputValue));
		expect(result).toEqual([input1, input3]);
		expect(filterFn).toHaveBeenCalledTimes(3);
	...
	it("should work for promise resolved to list in sequence", async () => {
		...
		const inputValue = [input1, input2, input3];
		const filterFn = getMockFn(jest)(n => n - 15, "filterFn");
		const result = await afilterGeneric(true)(filterFn)(createAsyncPromise(n => new Set(n))(inputValue));
		expect(result).toEqual([input1, input3]);
		expect(filterFn).toHaveBeenCalledTimes(3);
		mockFnExpectations(filterFn, 1, -11, input1, 0, inputValue);
		mockFnExpectations(filterFn, 2, 0, input2, 1, inputValue);
	...
	it("should reject for failed promise as input", async () => {
		...
		const filterFn = getMockFn(jest)(n => n < 10, "filterFn");
		try {
			await afilterGeneric()(filterFn)(createAsyncPromise(n => new Set(n), false)(inputValue));
	...
	it("should work with promise and value as input", async () => {
		...
		const result = await afilterGeneric()(filterFn)(
			new Set([input1, createAsyncPromise(increment)(input2), input3])
		);
	...
	it("should work with promise in parallel", async () => {
		...
		const filterFn = getMockFn(jest)(n => n < 10, "filterFn");
		const result = await afilterGeneric()(createAsyncPromise(filterFn))(new Set(inputValue));
	...
	it("should work with promise in sequence", async () => {
		...
		const inputValue = [input1, input2];
		const filterFn = getMockFn(jest)(n => n < 10, "filterFn");
		const result = await afilterGeneric(true)(createAsyncPromise(filterFn))(new Set(inputValue));
		expect(result).toEqual(inputValue);
		expect(filterFn).toHaveBeenCalledTimes(2);
		mockFnExpectations(filterFn, 1, true, input1, 0, inputValue);
		mockFnExpectations(filterFn, 2, true, input2, 1, inputValue);
	...
	it("should reject with rejected promise in parallel", async () => {
		...
		const filterFn = getMockFn(jest)(n => n < 10, "filterFn");
		try {
			await afilterGeneric()(createAsyncPromise(filterFn, false))(new Set(inputValue));
			expect(true).toBe(false);
		} catch (e) {
			expect(e).toBeInstanceOf(Error);
	...
	it("should reject with rejected promise in sequence", async () => {
		...
		const filterFn = getMockFn(jest)(n => n < 10, "filterFn");
		try {
			await afilterGeneric(true)(createAsyncPromise(filterFn, false))(new Set(inputValue));
			expect(true).toBe(false);
		} catch (e) {
			expect(e).toBeInstanceOf(Error);
			expect(e.message).toBe(getErrorMessage([input1, 0, inputValue]));
